...
```yaml
rootDir: "."
buildCommand: corepack enable && corepack pnpm install --frozen-lockfile && corepack pnpm --filter @nexcv/web build
startCommand: corepack pnpm --filter @nexcv/api start
```
...
corepack pnpm --filter @nexcv/web build
corepack pnpm --filter @nexcv/api start
```

`@nexcv/api` runs `tsx server.ts` for `start`. Make sure the host installs workspace dependencies from the repository root so local packages under `packages/` are linked.
...
Build the email worker artifact from the repository root:

```bash
corepack pnpm build:email-worker-lambda
```

Deploy the generated ZIP from `apps/workers/email-worker/dist/` to AWS Lambda with an SQS trigger on the email queue.
...
Build the CV import worker artifact from the repository root:

```bash
corepack pnpm build:cv-import-worker-lambda
```

Deploy the generated ZIP from `apps/workers/cv-import-worker/dist/` to AWS Lambda with an SQS trigger on the CV import queue.
...
Build the PDF worker artifact from the repository root:

```bash
corepack pnpm build:pdf-worker-lambda
```

Deploy the generated ZIP from `apps/workers/pdf-worker/dist/` to AWS Lambda with an SQS trigger on the PDF jobs queue.
...
Build the Lambda artifact from the repository root:

```bash
corepack pnpm build:pdf-lambda
```

Deploy the generated ZIP from `apps/workers/pdf-lambda/dist/` to AWS Lambda.
...
Build the OCR Lambda artifact from the repository root:

```bash
corepack pnpm build:ocr-lambda
```

Deploy the generated ZIP from `apps/workers/ocr-lambda/dist/` to AWS Lambda as `OCR_data_Extract`.
