...
export default function MyCvs() {
  ...
  const searchedDocuments = useMemo(() => {
    ...
    const base = activeTab === 'recent' ? documents.slice(0, 3) : activeTab === 'all' ? documents : [];
    if (!term) return base;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-slate-950 text-white">
      ...
      <AppSidebar />
      <main className="scrollbar-hide mx-auto min-w-0 max-w-7xl flex-1 px-4 pb-8 pt-5 sm:px-6 sm:pb-10 sm:pt-8 lg:h-dvh lg:overflow-y-auto lg:px-8">
        <section className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-[minmax(220px,1fr)_auto_auto] lg:w-[590px]">
            <label className="relative col-span-2 block sm:col-span-1">
              ...
              <input
                ...
                className="h-12 w-full rounded-xl border border-white/10 bg-white/[0.035] pl-4 pr-10 text-sm font-semibold text-white outline-none transition placeholder:text-slate-500 focus:border-violet-400"
        ...
        <section className="mt-6 grid gap-5 lg:grid-cols-[1fr_300px]">
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.035] shadow-2xl shadow-black/15">
            <div className="scrollbar-hide flex gap-1 overflow-x-auto border-b border-white/10 px-2 pt-2 sm:px-3 sm:pt-3">
              ...
              <FilterButton active={activeTab === 'drafts'} onClick={() => setActiveTab('drafts')} label="Drafts" count={0} />
          ...
          <aside className="grid gap-4 self-start">
            <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 shadow-xl shadow-black/10 sm:p-5">
              ...
              <div className="mt-4 grid gap-3">
                ...
                <OverviewRow icon={<CheckCircle2 size={16} />} label="Completed" value={documents.length} tone="emerald" />
                <OverviewRow icon={<FileText size={16} />} label="Drafts" value={0} tone="slate" />
...
function FilterButton({ active, onClick, label, count }: { active: boolean; onClick: () => void; label: string; count: number }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex min-w-max items-center gap-2 border-b-2 px-3 py-3 text-sm font-extrabold transition sm:px-4 sm:py-4 ${
