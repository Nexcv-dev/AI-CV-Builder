      ...
      <div className={`${DESIGN_CARD_CLASS} ${isDarkMode ? DESIGN_CARD_DARK : DESIGN_CARD_LIGHT}`}>
        ...
        <div className="space-y-5">
          ...
          <div className="relative" ref={fontDropdownRef}>
            ...
            <AnimatePresence>
              {isFontDropdownOpen && (
                <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} transition={{ duration: 0.2 }} className={`absolute z-100 w-full mt-2 max-h-72 overflow-y-auto overscroll-contain rounded-2xl shadow-xl py-2 border scrollbar-hide ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                  {fonts.map((f) => (
