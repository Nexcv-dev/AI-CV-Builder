...
- `server-models/` - MongoDB/Mongoose models and plan/quota helpers.
- `server-utils/` - auth, logging, billing, admin, and infrastructure helpers.
- `middlewares/` - security, session, timeout, and rate-limit middleware.
- `lambda-*` - Lambda source for PDF, PDF worker, CV import worker, OCR, and email worker.
- `scripts/` - build, validation, template release, and metadata scripts.
- `tests/` and `src/**/*.test.*` - Vitest server/frontend tests.
- `docs/` - operational, deployment, API, and feature documentation.
...
- Put route-specific HTTP behavior in `routes/` and reusable logic in `services/` or `server-utils/`.
...
- CV import: `POST /api/cv-import-jobs`, worker in `lambda-cv-import-worker/`, OCR in `lambda-ocr/`.
- PDF jobs: `POST /api/pdf-jobs`, worker in `lambda-pdf-worker/`, renderer in `lambda-pdf/`.
...
```powershell
$files = @('README.md','Free CV Builder\README.md') + (Get-ChildItem 'Free CV Builder\docs' -Filter *.md | ForEach-Object { $_.FullName })
$missing = @()
